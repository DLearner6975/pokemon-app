...
import { Bell, Calendar, ChevronDown, Filter, Link2, MoreHorizontal, Plus, Search, LogOut } from 'lucide-react'
...
export default function ProjectDashboard({ initialPokemon, allPokemon }: ProjectDashboardProps) {
  const [searchQuery, setSearchQuery] = useState('')
  const [currentPage, setCurrentPage] = useState(1)
  const [pokemonDetails, setPokemonDetails] = useState<Pokemon[]>(initialPokemon)
  ...
  const [user, setUser] = useState<{ name: string; location: string; avatarUrl: string } | null>(null)
  ...
  const filteredPokemon = allPokemon.filter(pokemon => {
    ...
    const matchesFilters = () => {
      if (filters.types.length > 0 && !details.types.some(t => filters.types.includes(t.type.name.toLowerCase()))) return false
      if (filters.generation.length > 0 && !filters.generation.includes(details.generation?.name.split('-')[1].toUpperCase() || '')) return false
      if (filters.abilities.length > 0 && !details.abilities.some(a => filters.abilities.includes(a.ability.name.toLowerCase()))) return false
      if (filters.habitat.length > 0 && !filters.habitat.includes(details.habitat?.name.toLowerCase() || '')) return false
      if (filters.shape.length > 0 && !filters.shape.includes(details.shape?.name.toLowerCase() || '')) return false
      if (filters.color.length > 0 && !filters.color.includes(details.color?.name.toLowerCase() || '')) return false
      if (details.base_experience < filters.baseExperience.min || details.base_experience > filters.baseExperience.max) return false
      if (filters.legendary && !details.is_legendary) return false
      if (filters.mythical && !details.is_mythical) return false

      const statsMatch = Object.entries(filters.stats).every(([stat, range]) => {
        const pokemonStat = details.stats.find(s => s.stat.name === stat)
        return pokemonStat && pokemonStat.base_stat >= range.min && pokemonStat.base_stat <= range.max
      })
  ...
  useEffect(() => {
    const fetchMissingDetails = async () => {
      const missingPokemon = currentPagePokemon.filter(
        pokemon => !pokemonDetails.some(detail => detail.name === pokemon.name)
      )

      if (missingPokemon.length > 0) {
        const newDetails = await Promise.all(missingPokemon.map(fetchPokemonDetails))
  ...
  const handleLogin = (username: string, password: string, avatarUrl: string) => {
